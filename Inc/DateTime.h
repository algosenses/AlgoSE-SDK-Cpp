#ifndef ALGOSE_DATETIME_H
#define ALGOSE_DATETIME_H
...
public:
	...
	explicit DateTime(const char* date);
...
//        return Date(t_.tv_sec/86400 + 719529);
    ...
    TimeStamp toTimeStamp() const {
        TimeStamp ts = { t_.tv_sec, t_.tv_usec };
        return ts;
    }

    void getFields(int& year, int& month, int& day, int& hour, int& minute, int& second, int& millis);
...
ALGOSE_API std::ostream& operator<<(std::ostream&, const DateTime&);

} // namespace AlgoSE

#endif // ALGOSE_DATETIME_H